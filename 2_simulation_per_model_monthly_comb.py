...
model_id = 0  # Misalnya kita pilih model 10
...
# Menghitung total jumlah prediksi untuk seluruh 12 bulan dan membaginya dengan 12 untuk rata-rata
total_prediksi = simulated_data['Jumlah_Prediksi'].sum()
rata_rata_per_bulan = total_prediksi / 12

# Buat DataFrame baru dengan hasil tersebut
result = pd.DataFrame({
    "Month": [1],  # Hanya 1 baris untuk bulan 1
    "Jumlah_Prediksi": [rata_rata_per_bulan]  # Rata-rata kebutuhan komponen per bulan
})
...
result.to_excel(f"{base_filename}{counter}.xlsx", index=False)
